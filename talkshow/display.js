...
function incrementLoaded(idx) {
  ...
  var id = queryUnderscore(query) + "_" + g_searchResults[idx].rank + g_searchResults[idx].source;
...
function incrementDisplayed(idx) {
  var query = g_searchResults[idx].query;
  var dashDiv = getQueryDiv(g_searchResults[idx].rank + g_searchResults[idx].source, query);
  if ($(dashDiv).css("color") == "rgb(255, 0, 0)") {
    ...
    $(barDiv).html($(barDiv).html() + s);
    dashDiv = getQueryDiv(g_searchResults[idx].rank + g_searchResults[idx].source, query);
  }
