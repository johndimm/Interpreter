...
function searchInternetAgain(query, nRequestedHits) {
  var half = parseInt(nRequestedHits/2);
  getBOSS(query, half); 
  getFlickr(query, half);
//return getBing(query, nRequestedHits);
}
...
//		url = url.replace("{farm-id}", imageInfo.farm)
//		  .replace("{server-id}", imageInfo.server)
//		  .replace("{id}", imageInfo.id)
//		  .replace("{secret}", imageInfo.secret)
//		  .replace("[mstzb]", "b");
  ...
  $.ajax({
    ...
    success: function (data, status) {
      ...
      for (i = 0; i < data.d.results.length; i++) {
        // Is this a good, new image?
        if (!addImgUrl(data.d.results[i].MediaUrl)) continue;

        // Create a new item in g_searchResults.		 
        var newSearchResults = new Object;
        newSearchResults.MediaUrl = data.d.results[i].MediaUrl;
        newSearchResults.query = query;
        newSearchResults.rank = i;
        newSearchResults.nDisplayed = 0;
        newSearchResults.priority = priority;
        newSearchResults.DisplayUrl = "http://" + data.d.results[i].DisplayUrl;
        newSearchResults.Title = data.d.results[i].Title;
        newSearchResults.loading = false;
        newSearchResults.loaded = false;
        newSearchResults.IsValid = true; // assume it's valid until we try to load it.
        newSearchResults.img;
